...
# Author: Anonymous
# -----
# Last Modified: Wednesday, 2nd October 2024 6:41:19 pm
...
from infl import infl_true, infl_segment_true, infl_icml, infl_sgd, infl_dit
...
def load_or_train_model(args, save_dir, logger):
    ...
    if args.skip_initial_train and os.path.exists(model_path):
        ...
        return train_and_save(
            ...
            logger=logger,
            flip_csv=None,  # Initially no flip CSV
        )
...
def main():
    ...
    # Step 4: Load existing fliped indices
    fliped_indices_file = os.path.join(
        full_save_dir, f"fliped_indices_{args.seed:03d}.csv"
    )
    fliped_indices_df = pd.read_csv(fliped_indices_file)
    fliped_indices = fliped_indices_df["fliped_indices"].values

    # Step 5: Remove influential samples from the fliped set
    corrected_indices = np.intersect1d(
        check_indices, fliped_indices
    )  # Find samples to be "corrected"
    updated_fliped_indices = np.setdiff1d(
        fliped_indices, corrected_indices
    )  # Update flip indices
    num_corrected = len(fliped_indices) - len(
        updated_fliped_indices
    )  # Number of corrections made

    logger.info(f"Corrected {num_corrected} labels")

    # Step 6: Save the new fliped indices to a CSV file
    # Modify filename to include check_{check}
    updated_fliped_indices_file = os.path.join(
        full_save_dir,
        f"fliped_indices_{args.type}_check_{args.check}_{args.seed:03d}.csv",
    )
    updated_fliped_indices_df = pd.DataFrame(
        {"fliped_indices": updated_fliped_indices}
    )
    updated_fliped_indices_df.to_csv(updated_fliped_indices_file, index=False)
    logger.info(f"Updated fliped indices saved to {updated_fliped_indices_file}")

    # Step 7: Retrain the model with updated flip CSV
    logger.info("Retraining model with updated flip indices")
    final_result = train_and_save(
        ...
        save_dir=f"{save_dir}",
        logger=logger,
        flip_csv=updated_fliped_indices_file,  # Use updated flip CSV file
    )

    # Step 8: Output final results to a TXT file
    check_number = len(check_indices)
    fix_number = num_corrected
    left_unfix_number = len(updated_fliped_indices)
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Run experiment with data fliping and influence calculation"
    )
    ...
    parser.add_argument(
        "--flip", type=float, default=10, help="Percentage of data to flip"
    )
    parser.add_argument(
        "--check",
        type=float,
