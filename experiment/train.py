...
# Author: Anonymous
# -----
# Last Modified: Wednesday, 2nd October 2024 6:41:37 pm
...
def generate_flip_indices(
    n_samples: int, flip_percentage: float, seed: int
) -> np.ndarray:
    """Generate indices of samples to be fliped."""
    np.random.seed(seed)
    num_to_flip = int(n_samples * flip_percentage / 100)
    return np.random.choice(n_samples, num_to_flip, replace=False)


def save_flip_indices(indices: np.ndarray, save_dir: str, seed: int):
    """Save flip indices to a CSV file."""
    os.makedirs(save_dir, exist_ok=True)
    filename = os.path.join(save_dir, f"fliped_indices_{seed:03d}.csv")
    pd.DataFrame({"fliped_indices": indices}).to_csv(filename, index=False)
    return filename


def load_flip_indices(filename: str) -> np.ndarray:
    """Load flip indices from a CSV file."""
    df = pd.read_csv(filename)
    return df["fliped_indices"].values


def apply_fliping(
    y_tr: torch.Tensor, flip_indices: np.ndarray, logger: logging.Logger
) -> torch.Tensor:
    """Apply fliping to the training labels."""
    y_tr[flip_indices] = 1 - y_tr[flip_indices]
    logger.info(f"Fliped {len(flip_indices)} samples")
    return y_tr
...
def train_and_save(
    ...
    logger=None,
    save_dir: str = None,
    flip_csv: str = None,  # New parameter for flip CSV file
    x_tr=None,
    y_tr=None,
    x_val=None,
    y_val=None,
    ...
    if flip_csv:
        ...
        flip_prefix = os.path.splitext(os.path.basename(flip_csv))[0] + "_"
    else:
        flip_prefix = ""

    dn = os.path.join(current_dir, save_dir)
    fn = os.path.join(dn, f"{flip_prefix}sgd{seed:03d}.dat")
    ...
    # Apply fliping if flip_csv is provided
    if flip_csv:
        try:
            flip_indices = load_flip_indices(flip_csv)
            y_tr = apply_fliping(y_tr, flip_indices, logger)
            logger.info(f"Applied fliping from file: {flip_csv}")
        except Exception as e:
            logger.error(f"Error applying fliping from {flip_csv}: {str(e)}")
            logger.error("Proceeding without fliping.")
    else:
        logger.info("No flip CSV provided. Proceeding without fliping.")
    ...
    # Save metrics to CSV
    csv_fn = os.path.join(dn, f"{flip_prefix}metrics_{seed:03d}.csv")
    pd.DataFrame(
        {
...
def _run_training(args, default_config, logger):
    train_and_save(
        ...
        flip_csv=args.flip_csv,  # Pass the flip_csv parameter
    )
...
def main():
    ...
    parser.add_argument(
        "--flip_csv",
        type=str,
        help="CSV file containing indices of samples to flip",
    )
    ...
    try:
        # Generate and save flip indices if --flip is specified
        if args.flip is not None and args.flip_csv is None:
            # We need to know n_tr to generate flip indices
            ...
            flip_indices = generate_flip_indices(n_tr, args.flip, args.seed)
            flip_csv = save_flip_indices(flip_indices, save_dir, args.seed)
            logger.info(
                f"Generated and saved flip indices for {args.flip}% of data to {flip_csv}"
            )
            args.flip_csv = flip_csv

        elif args.flip_csv is not None:
            logger.info(f"Using provided flip CSV file: {args.flip_csv}")
        else:
            logger.info("No fliping will be applied")
