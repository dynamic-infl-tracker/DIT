...
def infl_helper(
    ...
    dn, _ = get_file_paths(
        key, model_type, seed, save_dir=save_dir, flip_percentage=flip_percentage
    )
...
def infl_dit_first(
    ...
    infl_helper(
        key, model_type, seed, gpu, save_dir, 0, "dit_first", "dit", flip_percentage
    )
...
def infl_dit_middle(
    ...
    full_results = load_full_results(dn, seed, "dit")
...
def load_data(
    ...
    if flip_percentage:
        idx_csv_name = os.path.join(dn, f"fliped_indices_{seed:03d}.csv")
        # pd.DataFrame({"fliped_indices": fliped_indices}).to_csv(
        #     fliped_indices_fn, index=False
        # )
        logger.debug(f"Fliping {flip_percentage}% of the training data")
        fliped_indices = pd.read_csv(idx_csv_name)["fliped_indices"].values
        y_tr[fliped_indices] = 1 - y_tr[fliped_indices]
...
def get_file_paths(
    key, model_type, seed, infl_type=None, save_dir=None, flip_percentage=None
):
    dn = (
    ...
    # Modify filename if flip_percentage is set
    if flip_percentage is not None:
        fn = os.path.join(
            dn, f"fliped_indices_{seed:03d}_sgd{seed:03d}.dat"
        )  # New filename for fliped data
    else:
...
def infl_true(key, model_type, seed=0, gpu=0, save_dir=None, flip_percentage=None):
    ...
    dn, fn, gn = get_file_paths(
        key, model_type, seed, "true", save_dir, flip_percentage=flip_percentage
    )
...
def infl_segment_true(
    key, model_type, seed=0, gpu=0, save_dir=None, flip_percentage=None
):
...
def infl_sgd(key, model_type, seed=0, gpu=0, save_dir=None, flip_percentage=None):
    dn, fn, gn = get_file_paths(
        key, model_type, seed, "sgd", save_dir, flip_percentage=flip_percentage
    )
    device = f"cuda:{gpu}"
...
def infl_nohess(key, model_type, seed=0, gpu=0, save_dir=None, flip_percentage=None):
    dn, fn, gn = get_file_paths(
        key, model_type, seed, "nohess", save_dir, flip_percentage=flip_percentage
    )
    device = f"cuda:{gpu}"
...
def infl_icml(key, model_type, seed=0, gpu=0, save_dir=None, flip_percentage=None):
    logger = logging.getLogger(f"infl_icml_{key}_{model_type}")
    logger.info(f"Starting infl_icml computation for {key}, {model_type}, seed {seed}")

    dn, fn, gn = get_file_paths(
        key, model_type, seed, "icml", save_dir, flip_percentage=flip_percentage
    )
...
def infl_dit(
    key, model_type, seed=0, gpu=0, is_csv=True, save_dir=None, flip_percentage=None
):
    logger = logging.getLogger(f"infl_dit_{key}_{model_type}")
    logger.info(f"Starting infl_dit computation for {key}, {model_type}, seed {seed}")

    dn, fn = get_file_paths(
        key, model_type, seed, save_dir=save_dir, flip_percentage=flip_percentage
    )
    os.makedirs(dn, exist_ok=True)
    csv_fn = os.path.join(dn, f"infl_dit_full_{seed}.csv")

    logger = setup_logging(f"infl_dit_{key}_{model_type}", seed, dn, level=logging.INFO)
    logger.debug(f"Starting infl_dit computation for {key}, {model_type}, seed {seed}")
    ...
    torch.save(infl_list, os.path.join(dn, f"infl_dit_full_{seed:03d}.dat"))
    logger.debug(
        f"Full results saved to {os.path.join(dn, f'infl_dit_full_{seed:03d}.dat')}"
    )
    ...
    torch.save(first_epoch_diff, os.path.join(dn, f"infl_dit_first{seed:03d}.dat"))
    torch.save(middle_epoch_diff, os.path.join(dn, f"infl_dit_middle{seed:03d}.dat"))
    torch.save(last_epoch_diff, os.path.join(dn, f"infl_dit_last{seed:03d}.dat"))

    logger.info(f"Finished infl_dit computation for {key}, {model_type}, seed {seed}")
    logger.debug(f"Results saved to {csv_fn}")
...
def main():
    ...
    parser.add_argument(
        "--flip", type=float, help="percentage of training data to flip"
    )
    ...
    if args.type not in [
        ...
        raise ValueError(
            "Invalid influence type. Choose from 'true', 'segment_true', 'sgd', 'nohess', 'icml', 'dit', 'dit_first', 'dit_middle', 'dit_last', 'true_first', 'true_middle', 'true_last'."
        )
